...
a clone:
    ...
    go build -o icelake ./cmd/icelake
...
reported and nothing else. Each failure prints one stderr line beginning
`icelake: clickhouse mirror:`, naming the table and what went wrong; a daemon
whose log carries those lines has a mirror falling behind, not a lake losing
data. The one cost a sick server can impose is bounded
